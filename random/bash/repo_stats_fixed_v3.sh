...
# Configure API URLs based on the GitHub URL provided
if [[ "${GHE_URL}" == *"github.com"* ]]; then
  # Public GitHub
  API_URL="https://api.github.com"
  GRAPHQL_URL="https://api.github.com/graphql"
elif [[ "${GHE_URL}" == *"api.github"* ]]; then
  # Already an API URL
  API_URL="${GHE_URL}"
  GRAPHQL_URL="${GHE_URL}/graphql"
else
...
GetGitHubRateLimit() {
  ...
  if [ $? -ne 0 ] || [ -z "${RATE_LIMIT_RESPONSE}" ]; then
    echo "Unable to check rate limit. Assuming we're not being rate limited..."
    return 0
  fi

  # Extract the remaining count safely, defaulting to a high number if not found
  local REMAINING=$(echo "${RATE_LIMIT_RESPONSE}" | jq -r '.resources.graphql.remaining // 5000')
  
  # Check if the value is a number
  if [[ ! "${REMAINING}" =~ ^[0-9]+$ ]]; then
    echo "Rate limit check returned non-numeric value: ${REMAINING}, assuming not rate limited..."
    return 0
  fi
  
  if [ "${REMAINING}" -lt 100 ]; then
    local RESET_TIME=$(echo "${RATE_LIMIT_RESPONSE}" | jq -r '.resources.graphql.reset // 0')
    
    # Check if reset time is a number 
    if [[ ! "${RESET_TIME}" =~ ^[0-9]+$ ]]; then
      echo "Rate limit reset time is not a valid number: ${RESET_TIME}, continuing..."
      return 0
    fi
    
    local CURRENT_TIME=$(date +%s)
    local SLEEP_TIME=$((RESET_TIME - CURRENT_TIME + 60))
...
process_repo() {
  ...
  if [[ "${VERSION}" == "cloud" || $(echo "${VERSION:0:3} >= 2.17" | bc -l 2>/dev/null) == 1 ]]; then
  ...
  local RECORD_CT=$((COLLABORATOR_CT + PROTECTED_BRANCH_CT + PR_REVIEW_CT + MILESTONE_CT + ISSUE_CT + PR_CT + PR_REVIEW_COMMENT_CT + COMMIT_COMMENT_CT + ISSUE_COMMENT_CT + ISSUE_EVENT_CT + RELEASE_CT + PROJECT_CT))
  
  # Determine if there are migration issues
  local MIGRATION_ISSUE
  if [ "${RECORD_CT}" -ge 60000 ] || [ "${REPO_SIZE}" -gt 1500 ]; then
  ...
  # Output the stats
  echo "${OWNER},${REPO_NAME},${IS_EMPTY},${PUSHED_AT},${UPDATED_AT},${IS_FORK},${REPO_SIZE},${RECORD_CT},${COLLABORATOR_CT},${PROTECTED_BRANCH_CT},${PR_REVIEW_CT},${MILESTONE_CT},${ISSUE_CT},${PR_CT},${PR_REVIEW_COMMENT_CT},${COMMIT_COMMENT_CT},${ISSUE_COMMENT_CT},${ISSUE_EVENT_CT},${RELEASE_CT},${PROJECT_CT},${GHE_URL}/${OWNER}/${REPO_NAME},${MIGRATION_ISSUE}" >> "${OUTPUT_FILE_NAME}"
}
...
GetRepos() {
  ...
  if [ $? -eq 0 ]; then
    DebugJQ "${DATA_BLOCK}"
    local ERROR_MESSAGE=$(echo "${DATA_BLOCK}" | jq -r '.errors[0].message // ""')
    ...
    if [ -n "${ERROR_MESSAGE}" ] && [ "${ERROR_MESSAGE}" != "null" ]; then
      ...
      # Check if we have any repository nodes at all
      local REPO_COUNT=$(echo "${DATA_BLOCK}" | jq -r '.data.organization.repositories.nodes | length')
...
# Check GitHub version and authenticate user
echo "Checking GitHub instance version and authentication..."

# For GitHub Enterprise
if [[ "${GHE_URL}" != *"api.github.com"* ]]; then
  # Try to get API version first for GitHub Enterprise
  API_VERSION_DATA=$(curl -s -k -H "Authorization: token ${GITHUB_TOKEN}" "${API_URL}")
  if [ $? -ne 0 ]; then
    echo "Error: Could not connect to GitHub API endpoint: ${API_URL}"
    exit 1
  fi
  
  META_DATA=$(curl -s -H "Authorization: token ${GITHUB_TOKEN}" "${API_URL}/meta")
  ...
  if [ -z "${VERSION}" ] || [ "${VERSION}" == "null" ] || [ "${VERSION}" == "unknown" ]; then
    # If couldn't get version, check different endpoint
    VERSION_DATA=$(curl -s -H "Authorization: token ${GITHUB_TOKEN}" "${API_URL}")
    VERSION=$(echo "${VERSION_DATA}" | grep -oP '"installation_id":"\K[^"]*' || echo "unknown")
    if [ "${VERSION}" == "unknown" ]; then
      VERSION=$(curl -s -I -H "Authorization: token ${GITHUB_TOKEN}" "${API_URL}" | grep -i "X-GitHub-Enterprise-Version" | cut -d' ' -f2 | tr -d '\r' || echo "unknown")
    fi
  fi
else
  VERSION="cloud"
...
USER_DATA=$(curl -s -H "Authorization: token ${GITHUB_TOKEN}" "${API_URL}/user")
